...
class RefcocoReferringSegDataset(Dataset):
    ...
    def decode_mask(self, annotations_ids, image_info):
        ...
        for ann_id in annotations_ids:
            ...
            if len(ann["segmentation"]) == 0:
                ...
                masks.append(m)
                if "seg_img_path" in ann:
                    output_image.append(ann["seg_img_path"])
                continue
    ...
    def __getitem__(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            masks, output_image = self.decode_mask(data_dict['sampled_ann_id'], data_dict['image_info'])

            # print(data_dict['sampled_sents'])
            # print(data_dict['conversation'])
