...
def omg_llava_collate_fn(instances: Sequence[Dict],
    ...
    has_multi_image = any(inst.get('is_multi_image') is not None for inst in instances)
    ...
    for example in instances:
        ...
        if has_image:
            ...
            internvl_pixel_values.append(example['internvl_pixel_values'])
            output_images_pixel_values.append(example.get("output_images_pixel_values",[]))
    ...
    if has_image and not has_multi_image:
    ...
    elif has_image and has_multi_image:
        new_pixel_values = []
        image_nums = []
        for pixel_value in pixel_values:
            if type(pixel_value)==list:
                new_pixel_values.extend(pixel_value)
                image_nums.append(len(pixel_value))
            else:
                new_pixel_values.append(pixel_value)
                image_nums.append(1)
        data_dict['pixel_values'] = torch.stack(new_pixel_values)
        data_dict['internvl_pixel_values'] = internvl_pixel_values
        data_dict['has_multi_image'] = has_multi_image
        data_dict['image_nums'] = image_nums
    # elif has_image and has_multi_image:
    #     new_pixel_values = []
    #     for pixel_value in pixel_values:
    #         if type(pixel_value)==list:
    #             new_pixel_values.append(torch.stack(pixel_value))
    #         else:
    #             new_pixel_values.append(pixel_value.unsqueeze(0))
    #     data_dict['pixel_values'] = new_pixel_values
    #     data_dict['internvl_pixel_values'] = internvl_pixel_values
    #     data_dict['has_multi_image'] = has_multi_image
