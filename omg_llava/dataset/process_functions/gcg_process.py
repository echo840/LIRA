...
def refcocog_parse_annotations(example):
    ...
    annotations = {'labels': [], 'caption': [], 'masks': [], 'tokens_positive': [],
                   'file_name': example['img_file_name'], 'image': example['img_file_name'],"output_image":[]}
    ...
    for detail in example['refs']:
        ...
        if phrase.lower() in annotations['caption']:
            ...
            annotations['output_image'].append(detail["seg_img_path"])
    ...
    annotations['output_image'] = [annotations['output_image'][i] for i in sorted_indices]
...
def refcocog_conversation(caption, tokens_positive):
    ...
    def tag_caption(caption, tokens):
        for start, end in sorted(tokens, key=lambda x: x[0], reverse=True):
            caption = f"{caption[:start]}[SEG]<image_id><image> <p> {caption[start:end]} </p> [SEG]{caption[end:]}"
        return caption
...
def refcocog_preprocess(example):
    ...
    output_image = sort_by_start_index(output_image, phrase_order)
    ...
    example['output_image'] = output_image
...
def glamm_refcocog_map_fn(example):
    ...
    # print(conversation)
    # for i_num in range(len(output_image)):
    #     Image.open(output_image[i_num]).save("/home/kas/lz_new/omginternvl/test.jpg")
    #     print(labels[i_num])
...
def grandf_parse_annotations(example):
    ...
    for word, grounding in example["groundings"].items():
        ...
        annotations['output_image'].append(grounding['seg_img_path'])
...
def flickr_parse_annotations(example):
    annotations = {'bboxes': [], 'labels': [], 'bboxes_ignore': [], 'caption': example['caption'], 'masks': [],
                   'tokens_positive': [], 'image': example['file_name'],"output_image":[]}
    ...
    for ann in ann_info:
        ...
        annotations['output_image'].append(ann['seg_img_path'])
