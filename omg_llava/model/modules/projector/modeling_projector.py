...
class Naive_Proj(nn.Module):
    def __init__(self, config, rm_prior_embedding=False,
                 rm_query=False,add_cross=False,prefix=False,cross_head_num=32):
        super().__init__()
        ...
        self.prefix = prefix
        self.cross_head_num = cross_head_num
        ...
        if self.add_cross:
            ...
            print(f"Using Cross Attention Layer at Projector !!! {self.cross_head_num}")
            self.cross_beforellm = CrossAttentionLayer_Pre(
                d_model=config.llm_hidden_size,
                nhead=self.cross_head_num,
            )
        ...
        self.seperate_embed_internvl = nn.Embedding(1, config.llm_hidden_size)
    ...
    def forward(self, x, internvl_vit_embeds=None, onlyomg_pretrain=False):
        ...
        # valid_mask # (bs, q)#只找有用的query，即有对应分割物体的query
        # query_feat (bs, q, c)
        # clip_feature (bs, hw, c)
        # attn_map (bs, q, hw)
        bs, n_q = query_feat.shape[:2]

        layer_outputs = self.model(query_feat) #query_proj对齐clip feature，这里经过两层mlp对齐llm

        # filter
        clip_feature_out = clip_feature
        clip_feature_out = self.model_feat(clip_feature_out)# 两层mlp对齐llm
        ret = []
        ...
        if internvl_vit_embeds is None:
            for clip_feat, layer_output, keep in zip(clip_feature_out, layer_outputs, valid_mask):
                ...
                if self.rm_query:
                    ...
                    ret.append(torch.cat([self.seperate_embed_internvl.weight, clip_feat, self.seperate_embed.weight, valid_layer_output], dim=0))
        else:
            for internvl_vit_embed, clip_feat, layer_output, keep in zip(internvl_vit_embeds, clip_feature_out, layer_outputs, valid_mask):
                valid_layer_output = layer_output[keep]
                if self.add_cross_attn_layer:
                    valid_layer_output = self.query_cross_attn(
                        valid_layer_output.unsqueeze(1), clip_feat.unsqueeze(1),
                    ...
                    valid_layer_output = self.query_ffn(valid_layer_output)
                if self.add_cross:
                    valid_layer_output = self.cross_beforellm(valid_layer_output.unsqueeze(1), internvl_vit_embed.unsqueeze(1),)[:, 0]
                    clip_feat = self.cross_beforellm(clip_feat.unsqueeze(1), internvl_vit_embed.unsqueeze(1),)[:, 0]
                if self.rm_query:
                    ret.append(torch.cat([internvl_vit_embed, self.seperate_embed_internvl.weight, clip_feat], dim=0) + torch.mean(self.seperate_embed.weight) * 0.0 + torch.mean(valid_layer_output) * 0.0)
                else:
                    if onlyomg_pretrain:
                        ret.append(torch.cat([self.seperate_embed_internvl.weight, clip_feat, self.seperate_embed.weight, valid_layer_output], dim=0))
                    else:
                        if self.prefix:
                            ret.append(torch.cat([self.seperate_embed_internvl.weight, clip_feat, self.seperate_embed.weight, valid_layer_output, internvl_vit_embed], dim=0))
                        else:
                            ret.append(torch.cat([internvl_vit_embed, self.seperate_embed_internvl.weight, clip_feat, self.seperate_embed.weight, valid_layer_output], dim=0))
...
class ProjectorModel_OMG_LLaVA(PreTrainedModel):
    ...
    def __init__(self, config: ProjectorConfig_OMG_LLaVA, add_cross=False,prefix = False,cross_head_num=32) -> None:
        super().__init__(config)
        self.gradient_checkpointing = False

        self.rm_prior_embedding = False
        self.rm_query = False
        self.model = Naive_Proj(config, add_cross = add_cross,prefix=prefix,cross_head_num=cross_head_num)
    ...
    def forward(self, x, internvl_vit_embeds=None, onlyomg_pretrain=False):
        if self.gradient_checkpointing and self.training:
            layer_outputs = torch.utils.checkpoint.checkpoint(self.model, x, internvl_vit_embeds)
        else:
            layer_outputs = self.model(x, internvl_vit_embeds, onlyomg_pretrain=onlyomg_pretrain)
...
class CrossAttentionLayer_Pre(nn.Module):
