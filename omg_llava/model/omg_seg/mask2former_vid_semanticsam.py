...
class Mask2FormerVideoSemSamHead(AnchorFreeHead):
    ...
    def _forward_head_llm(self, decoder_out: Tensor, mask_feature: Tensor,
        ...
        if regions is not None:
            attn_mask = self._get_attn_mask_from_gt_mask(regions,attn_mask_target_size)
        else:
            if num_frames > 0:
    ...
    def _forward_head(self, decoder_out: Tensor, mask_feature: Tensor,
        ...
        decoder_out = self.transformer_decoder.post_norm(decoder_out)
        # shape (num_queries, batch_size, c)
        if isinstance(self.cls_embed, nn.Module):#false
            cls_pred = self.cls_embed(decoder_out)
        # shape (num_queries, batch_size, c)q
        mask_embed = self.mask_embed(decoder_out)  #query 经过三层linear
        # shape (num_queries, batch_size, h, w)
        mask_pred = torch.einsum('bqc,bchw->bqhw', mask_embed, mask_feature)
        if not isinstance(self.cls_embed, nn.Module):
            maskpool_embd = self.mask_pooling(x=mask_feature, mask=mask_pred.detach()) #每个mask>0处，特征值的平均
            maskpool_embd = self.mask_pooling_proj(maskpool_embd)#[1, 300, 256]
    ...
    def forward_llm_seg(self, hidden_states, batch_idxs,regions=None):
    ...
    def llm_seg_loss(self, mask_pred_list, gt_masks):
        ...
        for mask_pred in mask_pred_list:
            # #add
            # gt_masks = gt_masks.unsqueeze(1)
            # sampled_mask_gt = gt_masks.view(gt_masks.shape[0], -1)
            # sampled_mask_pred = mask_pred.view(mask_pred.shape[0], -1)
            # #add
    ...
    def forward(self, x: List[Tensor], batch_data_samples: SampleList,
        ...
        if isinstance(batch_data_samples[0], TrackDataSample):
            ...
            for data_sample in batch_data_samples:
                batch_img_metas.append(data_sample.metainfo) #[{'img_shape': (1024, 1024), 'ori_shape': (1024, 1024), 'batch_input_shape': (1024, 1024)}]
            num_frames = 0
        batch_size = len(batch_img_metas)
        #四层特征，最后一层作为mask_features
        mask_features, multi_scale_memorys = self.pixel_decoder(x)#[1, 192, 256, 256],[1, 384, 128, 128],[1, 768, 64, 64],[1, 1536, 32, 32]->[1, 256, 256, 256],[[1, 256, 32, 32],[1, 256, 64, 64],[1, 256, 128, 128]]
        ...
        decoder_positional_encodings = []
        for i in range(self.num_transformer_feat_level):
            decoder_input = self.decoder_input_projs[i](multi_scale_memorys[i]) # [1, 256, 32, 32]
            # shape (batch_size, c, h, w) -> (batch_size, h*w, c)
            decoder_input = decoder_input.flatten(2).permute(0, 2, 1)   # [1, 1024, 256]
            if num_frames > 0:
            ...
            decoder_positional_encoding = self.decoder_positional_encoding(
                mask)
            decoder_positional_encoding = decoder_positional_encoding.transpose(
                1, 2).flatten(2).permute(0, 2, 1)#[1, 1, 256, 32, 32]->[1, 256, 1, 32, 32]->[1, 1024, 256]
            decoder_inputs.append(decoder_input)
            decoder_positional_encodings.append(decoder_positional_encoding)
        #[1, 1024, 256]  [1, 4096, 256]  [1, 16384, 256]
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        if isinstance(data_sample, TrackDataSample):
            ...
            img_shape = data_sample.metainfo['batch_input_shape'] #(1024, 1024)
            num_frames = 0 
        if return_mask_features:
        ...
        if num_frames > 0:
            mask_pred_results = mask_pred_results.flatten(1, 2)#[1, 300, 256, 256]
    ...
    def prepare_sam_query(self, points, w, h):
        ...
        input_bbox_embed = inverse_sigmoid(known_bbox_expand)#torch.Size([63, 1, 4])

        input_label_embed = input_label_embed.repeat_interleave(
            self.num_mask_tokens,
            1) + self.mask_tokens.weight.unsqueeze(0).repeat(
            input_label_embed.shape[0], input_label_embed.shape[1], 1)#torch.Size([63, 1, 256])
        input_bbox_embed = input_bbox_embed.repeat_interleave(
            self.num_mask_tokens, 1) #torch.Size([63, 1, 4])
        ...
        query_embed = coordinate_to_encoding(input_query_bbox.sigmoid())
        query_embed = self.pos_linear(query_embed)
        return input_query_label, query_embed  # (N, 1, C) #torch.Size([63, 1, 256])  torch.Size([63, 1, 256])
    ...
    def _forward_head_sam(self, decoder_out: Tensor, mask_feature: Tensor,
        ...
        Returns:
            tuple: A tuple contain three elements.
                ...
                - num_frames: How many frames are there in video.
        """
        #query feat      mask featuer
        decoder_out = self.transformer_decoder.post_norm(decoder_out)
        mask_embed = self.mask_embed(decoder_out)

        # shape (num_queries, batch_size, h, w)
        mask_pred = torch.einsum('bqc,bchw->bqhw', mask_embed, mask_feature) #torch.Size([61, 1, 256, 256])
    ...
    def forward_point_prompt(self, points, batch_idxs, width, height):
        ...
        ) # (N, 1, c) 0embedding+mask_token      boxes2embed再linear
        ...
        decoder_inputs = []
        decoder_positional_encodings = []
        #multi_scale_memorys+level_embed->decoder_input    decoder_positional_encodings
        for i in range(self.num_transformer_feat_level):
            decoder_input = self.decoder_input_projs[i](multi_scale_memorys[i])
            # shape (batch_size, c, h, w) -> (batch_size, h*w, c)
            decoder_input = decoder_input.flatten(2).permute(0, 2, 1)
            level_embed = self.level_embed.weight[i].view(1, 1, -1)# 
            decoder_input = decoder_input + level_embed#torch.Size([8, 1024, 256]) + torch.Size([1, 1, 256])
        ...
        mask_pred_list = []
        #query->norm->mask_emb->*maskfeature->mask_pred   mask_pred->resize->atten mask
        mask_pred, attn_mask = self._forward_head_sam(
            query_feat, mask_features, multi_scale_memorys[0].shape[-2:],
            num_frames=num_frames
        )
