...
class OMGSegVisualEncoder(Mask2formerVideo):
    ...
    def forward_llm_seg(self, hidden_states, batch_idxs,regions=None):
        ...
        mask_pred_results = self.panoptic_head.forward_llm_seg(
            hidden_states, batch_idxs,regions=regions,
        )
    ...
    def forward(self, images, output_hidden_states=True):
        ...
        images = images.to(self.dtype)  #[1,3,1024,1024]

        img_shape = images.shape[-2:]
        # last scale for ConvNext-L
        feat_shape = [item // 32 for item in img_shape]  #[32, 32]
        if self.pixel_shuffle_down_ratio is not None:
            feat_shape = [item // self.pixel_shuffle_down_ratio for item in feat_shape] #[16, 16]
        ...
        feats = self.extract_feat(batch_inputs)#[1, 192, 256, 256]    [1, 384, 128, 128]   [1, 768, 64, 64]    [1, 1536, 32, 32]  
        llava_clip_feat = self.llava_visual_feat(feats)   #torch.Size([1, 256, 6144])

        # directly do panoptic segmentation
        #output [1, 300, 134]  [1, 300, 1024, 1024]  [1, 300, 256]   [1, 300, 256]  [1, 300, 1]   [1, 256, 256, 256]
    ...
    def panoptic_postprocess(self, mask_cls, mask_pred, query_feat, feat_size=[320, 320]):
        ...
        cur_scores = scores_foreground #每个query的类别最大logits？
        cur_masks = mask_pred #每个query，每个pixel预测的概率

        cur_query_feat = query_feat
        # smooth_ratio = 0.05
        smooth_ratio = 0.5
        # use 0.5 as the smooth score
        cur_prob_masks = (cur_scores.view(-1, 1, 1) * (1 - smooth_ratio) + smooth_ratio) * cur_masks #300,1,1 * 300,1024,1024
        #类别概率*mask概率=该类别mask概率？
        ...
        attn_mask = attn_mask.flatten(1)  # 每个query所应该关注图像的pixel mask，第几个query的id 对应cur_mask_ids
        ...
        return pixel_query, attn_mask #每个pixel处的特征（由每个pixel上在query上的概率，乘以query特征） 每个query所应该关注图像的pixel mask
