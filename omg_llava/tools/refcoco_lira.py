...
from omg_llava.dataset.utils.grefer import G_REFER
...
class RefcocoReferringSegDataset(Dataset):
    ...
    def json_file_preprocess(self, data_path):
        ...
        if "grefcoco" in self.dataset_name:
                    ...
                    for ann_id_i in ann_id:
                        ...
                        if len(ann["segmentation"]) == 0:
                            ...
                            if isinstance(ann['segmentation'], dict):
                                rle = mask.frPyObjects(ann['segmentation'], *ann['segmentation']['size'])
    ...
    def get_questions(self, text_require_infos):
        ...
        for sent in sampled_sents:
            ...
            # ret.append("Please segment {}.".format(sent))
            ret.append("Please segment {}.".format(sent.lower()))
            # ret.append("Please segment {}.".format(sent.lower().capitalize()))
            # ret.append("Please segment <p> {} </p>.".format(sent))
        return ret

    def filter_data_dict(self, data_dict):
        names = ['pixel_values', 'masks', 'ori_size', 'questions','internvl_pixel_values',"ori_image","image_file"]
    ...
    def __getitem__(self, index):
        ...
        if data_dict.get('image', None) is not None:
            ...
            if self.use_thumbnail_ is None:
                internvl_images = dynamic_preprocess(ori_image, min_num=4, max_num=12,
                                            image_size=448, use_thumbnail=True)
            elif self.use_thumbnail_ == True:
                internvl_images = dynamic_preprocess(ori_image, min_num=1, max_num=1,
                                            image_size=448, use_thumbnail=True)
...
def main():
    ...
    if cfg.get("pretrained_pth",None) is not None:
    ...
    dataset = RefcocoReferringSegDataset(
        ...
        use_thumbnail_=cfg.get("use_thumbnail",None),
        # debug=True,
    )
    json_save = []
    results = []
    ...
    for i in tqdm.tqdm(per_rank_ids, desc=f'Rank {rank}'):
        ...
        if args.prompt_template:
            for text in texts:
                template = PROMPT_TEMPLATE[args.prompt_template]
                if args.prompt_template == "internvl2_chat":
        ...
        internvl_vit_embeds = []
        internvl_vit_embed = model.extract_feature(data_sample['internvl_pixel_values'].to(model.vlm_enc.dtype).cuda())
        ...
        if isinstance(visual_outputs, list) or isinstance(visual_outputs, tuple)\
            ...
            pixel_values = projector(visual_outputs, internvl_vit_embeds)
        else:
        ...
        for idx_inp, inputs in enumerate(batch_inputs):
            ...
            predict = tokenizer.decode(generate_output.sequences[0]).strip()
            ...
            # intersection, union, accuracy_iou = 0.0, 0.0, 0.0
            for prediction, target in zip(masks, _target):
        ...
        try:
            ...
            print(_target.sum())
            print(masks.sum())
            intersection, union = np.array([intersection,intersection]), np.array([union,union])
            accuracy_iou = np.array([accuracy_iou, accuracy_iou]) / target_masks.shape[0]
    ...
    if get_rank() == 0:
        ...
        dir_file = args.pth_model.split('/')[-1].replace(".pth","")
        if not os.path.exists(f"./results/{dir_file}"):
            os.makedirs(f"./results/{dir_file}")
            print(f"make ./results/{dir_file}")
        with open(f'./results/{dir_file}/{args.save_name}{args.dataset}_{args.split}.txt', 'w', encoding='utf-8') as file:
            file.write(f'ciou:{class_iou}; giou:{global_iou}.')
