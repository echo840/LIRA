...
class EvaluateChatHook(Hook):
    ...
    def _eval_images(self,
        ...
        for sample_image, sample_input in zip(self.evaluation_images,
            ...
            if self.is_internvl:
                internvl_vit_embeds = []
                #add
                internvl_images = dynamic_preprocess(sample_image, min_num=1, max_num=12,
                ...
                internvl_pixel_value = torch.stack(internvl_pixel_values).to(device)
                num_patches = internvl_pixel_value.size(0)
                #add
                internvl_vit_embed = model.extract_feature(internvl_pixel_value.to(model.vlm.vision_model.dtype))
                internvl_vit_embed = internvl_vit_embed.reshape(-1, model.vlm.language_model.config.hidden_size)
            ...
            if isinstance(visual_outputs, list) or isinstance(visual_outputs, tuple)\
                ...
                if self.is_internvl:
                    pixel_values = model.projector(visual_outputs, internvl_vit_embeds)
                else:
    ...
    def _generate_samples(self,
        ...
        if self.is_internvl:
            is_checkpointing = model.vlm.language_model.is_gradient_checkpointing
            use_cache = model.vlm.language_model.config.use_cache
        else:
        ...
        if self.is_internvl:
            model.vlm.language_model.config.use_cache = True
        else:
        ...
        if self.is_internvl:
            model.vlm.language_model.config.use_cache = use_cache
        else:
