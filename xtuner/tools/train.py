...
import sys
sys.path.insert(0,"/home/kas/lz_new/LIRA")
...
def main():
    ...
    word_dirs_save_path = "./work_dirs/"+args.config.split('/')[-1].replace(".py","")
    if os.path.exists(word_dirs_save_path):
        pth_files = [os.path.join(word_dirs_save_path, file) for file in os.listdir(word_dirs_save_path) if file.endswith(".pth")]
        if len(pth_files)>0:
            max_iter_file = max(pth_files, key=lambda x: int(x.split('_')[-1].split('.')[0]))
            if args.resume is None:
                args.resume=max_iter_file
                print(f"resume from {max_iter_file}")
